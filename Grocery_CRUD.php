...
class grocery_CRUD_Field_Types
{
	...
	protected function change_list_value($field_info, $value = null)
	{
		...
		switch ($real_type) {
			...
			break;
			case 'text':
				$value = $this->character_limiter(strip_tags($value),180,"...");
			break;
			case 'date':
...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function get_total_results()
	{
		...
		if(!empty($this->relation))
			foreach($this->relation as $relation)
				$this->basic_model->join_relation($relation[0],$relation[1],$relation[2],$relation[5]);
	...
	{
		/* Softart */
		if($this->unset_ajax_list)
		{
			die('You have $crud->unset_ajax_list in your method');
		}
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_layout()
	{
		...
		if ($this->unset_jquery) {
			...
			// unset($js_files[sha1($this->default_theme_path.'/bootstrap/js/datagrid/gcrud.datagrid.js')]);
			// $this->set_js('admincp/assets/grocery_crud/themes/bootstrap/js/datagrid/gcrud.datagrid_2.js');
			// // $this->set_js($this->default_theme_path.'/bootstrap/js/datagrid/gcrud.datagrid_2.js');
			// unset($js_files[sha1($this->default_theme_path.'/bootstrap/js/datagrid/gcrud.datagrid.js')]);
			// unset($js_files[sha1($this->default_theme_path.'/bootstrap/js/datagrid/list.js')]);
		}
...
/* Softart */
	protected $unset_ajax_list			= false;
	...
	public function unset_ajax_list()
	{
		$this->unset_ajax_list = true;
		...
		if($this->columns_checked === false)
		{
			...
			foreach($this->columns as $col_num => $column)
			{
				...
				if(isset($this->relation[$column]))
				{
					...
					$new_column = $this->relation[$column][0];
				...
				else
				{
					if(!empty($this->relation))
					{
						...
						foreach($this->relation as $relation)
						{
							...
							{
								//$new_column = $relation[1].'.'.$relation[2];
								...
								$column = $new_column;
								$this->columns[$col_num] = $new_column;
							}elseif(is_array($relation[5]) && count($relation[5]) > 0) {
								foreach ($relation[5] as $name => $col) {
									if($name == $column) {
										$relation_f = $relation[1].'.'.$col;
									}
								}
							}
						}
				...
				if(isset($this->display_as[$column]))
					$this->columns[$col_num] = (object)array('field_name' => $column, 'display_as' => $this->display_as[$column], 'relation_field' => $relation_f);
				elseif(isset($field_types[$column]))
					$this->columns[$col_num] = (object)array('field_name' => $column, 'display_as' => $field_types[$column]->display_as, 'relation_field' => $relation_f);
				else
					$this->columns[$col_num] = (object)array('field_name' => $column, 'display_as' =>
						ucfirst(str_replace('_',' ',$column)), 'relation_field' => $relation_f);
	...
	public function render()
	{
		...
		switch ($this->state_code) {
			...
			case 7://ajax_list
				...
				if($this->unset_ajax_list)
				{
					throw new Exception('Oooo No, this is custom query', 14);
					die();
				}
	 ...
	 * @param array $get_culumn - key is display_as name, value is get column from relation table
     * @return Grocery_CRUD
	 */
	public function set_relation($field_name , $related_table, $related_title_field, $where_clause = null, $order_by = null, $get_columns = null)
	{
		//$this->relation[$field_name] = array($field_name, $related_table,$related_title_field, $where_clause, $order_by, $as, $get_column);
		$this->relation[$field_name] = array($field_name, $related_table,$related_title_field, $where_clause, $order_by, $get_columns);
		return $this;
	}
