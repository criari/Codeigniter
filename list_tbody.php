...
<?php foreach($list as $num_row => $row){ ?>
    <tr>
        ...
        <?php foreach($columns as $column){ ?>
            <td>
                <?php 
                    if($row->{$column->field_name} != '') {
                        $img = explode('<a ', $row->{$column->field_name});
                        if($column->field_name == 'image' && (count($img) < 2)) {
                            echo '<a href="'.$row->{$column->field_name}.'" class="image-thumbnail"><img src="'.$row->{$column->field_name}.'" height="50px"></a>';
                        }else {
