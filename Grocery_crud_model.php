...
class Grocery_crud_model  extends CI_Model  {
    ...
    function get_list()
    {
    	...
    	if(!empty($this->relation))
    	{
    		...
    		{
    			list($field_name , $related_table , $related_field_title, $get_columns) = $relation;
    			...
    			{
                    if(is_array($get_columns) && count($get_columns) > 0) {
                        foreach ($get_columns as $name => $column) {
                            $select .= ", `$related_table`.`$column` AS `$name`";
                        }
                    }else {
    ...
    function join_relation($field_name , $related_table , $related_field_title, $get_columns)
    {
		...
		if($related_primary_key !== false)
		{
			...
			$this->db->join( $related_table.' as '.$related_table , "$related_table.$related_field_title = {$this->table_name}.$field_name",'left');

			$this->relation[$field_name] = array($field_name , $related_table , $related_field_title, $get_columns);
